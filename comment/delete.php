...
if(count($_POST) > 0){
    if(isset($_POST['postID']) && isset($_POST['userID']) && $_SESSION['id'] == query('SELECT userID FROM comments WHERE userID = ? AND postID = ?', [$_GET['userID'], $_GET['postID']])->fetch()['userID']){
        query('DELETE FROM comments WHERE postID = ? AND userID = ?', [$_POST['postID'], $_POST['userID']]);
        header('Location: ../index.php');

        } else echo 'Not all fields are filled out';
}

if($_SESSION['id'] != query('SELECT userID FROM comments WHERE userID = ? AND postID = ?', [$_GET['userID'], $_GET['postID']])->fetch()['userID'] && $_SESSION['admin'] != true){
                ...
                <div class="col-lg-6">
                    <form method="POST">
                        <div class="text-center">
                            ...
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <a href="../post/index.php?id=<?php echo $_GET['postID']; ?>" class="btn btn-primary btn-xl"><strong>Cancel</strong></a>
                        </div>
                    </form>
                </div>
            </div>
